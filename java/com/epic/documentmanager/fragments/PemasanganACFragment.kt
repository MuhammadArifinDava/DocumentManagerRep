...
class PemasanganACFragment : Fragment() {
    ...
    private var editingDocument: PemasanganAC? = null

    // Views
    private lateinit var etNama: EditText
    private lateinit var etAlamat: EditText
    private lateinit var etNoTelepon: EditText
    private lateinit var etJenisAC: EditText
    ...
    companion object {
        ...
        fun newInstance(document: PemasanganAC? = null): PemasanganACFragment {
            val fragment = PemasanganACFragment()
            val args = Bundle()
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_pemasangan_ac, container, false)
        initViews(view)
        setupRecyclerView()
        setupObservers()
        setupClickListeners()
        ...
        arguments?.getSerializable(ARG_EDIT_DOCUMENT)?.let { document ->
            editingDocument = document as PemasanganAC
            isEditMode = true
    ...
    private fun initViews(view: View) {
        ...
        etJenisAC = view.findViewById(R.id.etJenisAC)
    ...
    private fun validateForm(): Boolean {
        val fields = mapOf(
            ...
            "Jenis AC" to etJenisAC.text.toString().trim(),
    ...
    private fun saveDocument() {
        ...
        if (localImages.isNotEmpty()) {
            ...
            documentViewModel.uploadFiles(uris, Constants.DOC_TYPE_PEMASANGAN_AC, fileNames)
        } else {
    ...
    private fun saveDocumentWithImages(uploadedUrls: List<String>) {
        ...
        val document = PemasanganAC(
            id = editingDocument?.id ?: "",
            uniqueCode = editingDocument?.uniqueCode ?: CodeGenerator.generateCodeForPemasanganAC(),
            nama = etNama.text.toString().trim(),
            alamat = etAlamat.text.toString().trim(),
            noTelepon = etNoTelepon.text.toString().trim(),
            jenisAC = etJenisAC.text.toString().trim(),
        ...
        documentViewModel.savePemasanganAC(document)
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_IMAGE_PICK && resultCode == Activity.RESULT_OK) {
            data?.data?.let { uri ->
                val imageName = "AC_${System.currentTimeMillis()}.jpg"
